...
어느 학교에서 학생 1,000명을 대상으로 다음과 같은 민감한 설문조사를 진행한다고 하자.
...
그렇다면 전체 통계는 어떻게 계산할 수 있을까?

예를 들어 1,000명의 학생 중 600명이 "네"라고 답했다고 가정하자. 동전이 공정하다면 전체 학생 중 약 500명은 앞면이 나와 자동으로 "네"라고 답했을 것으로 기대할 수 있다. 따라서 나머지 100명은 실제로 부정행위를 했기 때문에 "네"라고 답한 학생으로 볼 수 있다.

이처럼 Local Differential Privacy에서는 개별 사용자의 응답을 먼저 무작위화(randomization)하여 프라이버시를 보호한다. 서버는 원본 응답을 알 수 없지만, 충분히 많은 사용자의 데이터를 수집하면 전체 집단의 통계적 특성은 여전히 추정할 수 있다.
