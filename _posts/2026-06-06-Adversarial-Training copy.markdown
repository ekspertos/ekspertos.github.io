...
1. Temperature를 \(T\)로 설정하여 teacher network **A**를 학습한다.
2. 학습된 teacher network **A**를 이용하여 soft label을 생성한다.
3. 동일한 temperature \(T\)를 사용하여 soft label로 student network **B**를 학습한다.
4. 테스트 시에는 temperature를 \(T=1\)로 설정하여 추론을 수행한다.

일반적인 Distillation은 teacher network가 학습한 클래스 간의 관계(class similarity)를 student network에 효과적으로 전달하기 위해 사용된다. Teacher의 soft label에는 단순히 정답 클래스뿐만 아니라 다른 클래스들과의 상대적인 유사도 정보도 포함되어 있기 때문이다.

본래 Distillation은 teacher network보다 더 작은 student network를 사용하여 모델 압축(model compression)을 수행하는 것이 주된 목적이다. Student network는 제한된 capacity를 가지더라도 teacher가 제공하는 soft label을 학습함으로써 단순한 hard label만으로 학습했을 때보다 더 우수한 성능을 얻을 수 있다.

반면 Defensive Distillation에서는 teacher network **A**와 student network **B**가 일반적으로 동일한 구조와 capacity를 사용한다. 따라서 모델 압축이라는 원래의 목적보다는 soft label을 이용하여 더 smooth한 decision boundary를 학습하고 adversarial attack에 대한 robustness를 높이는 것을 목표로 한다.
...
Defensive Distillation은 gradient 정보를 이용하여 adversarial example을 생성하는 FGSM 공격에 대해 높은 방어 성능을 보이는 것으로 보고되었다.
...
Defensive Distillation에서는 큰 temperature $T$를 사용하여 학습한 뒤 테스트 시에는 $T=1$을 사용한다. 이 과정에서 모델의 logits 값이 매우 커지게 되며, 결과적으로 softmax 출력은 다음과 같이 포화(saturation)된다.

$$
[1,0,0,\ldots,0]
$$

즉 정답 클래스의 확률은 1에 매우 가까워지고 나머지 클래스의 확률은 0에 매우 가까워진다.
그리고 이 경우 역전파 되는 $y_i - t_i$ 값 또한 매우 작아진다. 따라서 FGSM은 adversarial example을 생성하는 데 필요한 유의미한 gradient 정보를 얻지 못하게 된다.

이러한 현상을 **Gradient Masking**이라고 부르며, Defensive Distillation이 FGSM에 강한 방어 성능을 보이는 주된 이유이다.
...
이것이 CW attack의 핵심 아이디어이다.

### 2.4. How CW attack works



<img src="https://ekspertos.github.io/assets/img/review/Adversarial/CW_Attack.jpg" width="400">

우리가 구하고 싶은 문제를 최적화 문제로 적으면 아래와 같이 적을 수 있다.

$$
\begin{aligned}
\min_{\delta} \quad & D(x, x+\delta) \\
\text{subject to} \quad & C(x+\delta)=t, \\
& x+\delta \in [0,1]^n
\end{aligned}
$$

그러나 logit을 비교하는 C(x+\delta)
...
$$
\begin{aligned}
\min_{\delta} \quad & D(x, x+\delta) + c \cdot f(x+\delta) \leq 0, \\
& x+\delta \in [0,1]^n
\end{aligned}
$$
...
$$
\begin{aligned}
\min_{\delta} \quad & \| \delta \|_p + c \cdot f(x+\delta) \leq 0, \\
& x+\delta \in [0,1]^n
\end{aligned}
$$
